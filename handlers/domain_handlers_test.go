...
var _ = Describe("Domain Handlers", func() {
	...
	Describe("Upsert", func() {
		...
		Context("when the DB has no domains table", func() {
			BeforeEach(func() {
				fakeDomainDB.UpsertDomainReturns(models.ErrNoTable)
			})

			It("logs and writes the exit channel", func() {
				Eventually(logger).Should(gbytes.Say("failed-domains-table-does-not-exist"))
	...
	Describe("Domains", func() {
		...
		Context("when the DB has no domains table", func() {
			BeforeEach(func() {
				fakeDomainDB.DomainsReturns([]string{}, models.ErrNoTable)
			})
